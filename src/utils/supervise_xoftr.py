...
def supervise_coarse_xoftr(data, config):
...
def supervise_fine_xoftr(data, config):
...
def create_conf_prob(points, h0, w0, h1, w1, kernel_size=5, sigma=1):
    ...
    conf_prob = F.conv2d(impulses.view(-1, 1, h1, w1), gaussian_kernel, padding=kernel_size // 2).view(-1, h0 * w0,
                                                                                                       h1 * w1)
...
def spvs_fine(data, config):
    ...
    if 'scale0' in data:
        scale0 = scale * data['scale0']  # Assuming [N] tensor
    else:
        scale0 = torch.full((N,), float(scale), device=device)

    if 'scale1' in data:
        scale1 = scale * data['scale1']  # Assuming [N] tensor
    else:
        scale1 = torch.full((N,), float(scale), device=device)
    ...
    if len(b_ids) == 0:
        data.update({"conf_matrix_f_gt": torch.zeros(0, W_f * W_f, W_f * W_f, device=device)})

        if 'conf_matrix_fine' not in data:
            data['conf_matrix_fine'] = torch.zeros(0, W_f * W_f, W_f * W_f, device=device)
        return
    ...
    # (unhandled edge case: points with 0-depth will be warped to the left-up corner)
    _, w_pt0_i = warp_kpts_fine(grid_pt0_i, data['depth0'], data['depth1'], data['T_0to1'], data['K0'], data['K1'],
                                b_ids)
    _, w_pt1_i = warp_kpts_fine(grid_pt1_i, data['depth1'], data['depth0'], data['T_1to0'], data['K1'], data['K0'],
                                b_ids)
    w_pt0_f = w_pt0_i / scale1[b_ids].view(-1, 1, 1)
    w_pt1_f = w_pt1_i / scale0[b_ids].view(-1, 1, 1)
